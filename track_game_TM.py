...
def main():
    ...
    parser.add_argument('-t','--template',required=True,help="path to ball template")
      ...
      with tf.Session(graph=detection_graph) as sess:
       ...
       while (True):
          ...
          if counter % 1 == 0: #each frame
              ...
              #target = cv2.imread("./FIFA4/image02038.jpg")
              cv2.namedWindow('template image', cv2.WINDOW_NORMAL)
              cv2.imshow("template image", template)
              #cv2.namedWindow('target image', cv.WINDOW_NORMAL)
              ...
              cv2.namedWindow("match-" + np.str(method), cv2.WINDOW_NORMAL)
              cv2.imshow("match-" + np.str(method), image_np)
